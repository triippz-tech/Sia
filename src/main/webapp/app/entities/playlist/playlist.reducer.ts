import axios from 'axios';
import { ICrudGetAction, ICrudGetAllAction, ICrudPutAction, ICrudDeleteAction } from 'react-jhipster';

import { cleanEntity } from 'app/shared/util/entity-utils';
import { REQUEST, SUCCESS, FAILURE } from 'app/shared/reducers/action-type.util';

import { IPlaylist, defaultValue } from 'app/shared/model/playlist.model';

export const ACTION_TYPES = {
  FETCH_PLAYLIST_LIST: 'playlist/FETCH_PLAYLIST_LIST',
  FETCH_PLAYLIST: 'playlist/FETCH_PLAYLIST',
  CREATE_PLAYLIST: 'playlist/CREATE_PLAYLIST',
  UPDATE_PLAYLIST: 'playlist/UPDATE_PLAYLIST',
  DELETE_PLAYLIST: 'playlist/DELETE_PLAYLIST',
  RESET: 'playlist/RESET'
};

const initialState = {
  loading: false,
  errorMessage: null,
  entities: [] as ReadonlyArray<IPlaylist>,
  entity: defaultValue,
  updating: false,
  updateSuccess: false
};

export type PlaylistState = Readonly<typeof initialState>;

// Reducer

export default (state: PlaylistState = initialState, action): PlaylistState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.FETCH_PLAYLIST_LIST):
    case REQUEST(ACTION_TYPES.FETCH_PLAYLIST):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
        loading: true
      };
    case REQUEST(ACTION_TYPES.CREATE_PLAYLIST):
    case REQUEST(ACTION_TYPES.UPDATE_PLAYLIST):
    case REQUEST(ACTION_TYPES.DELETE_PLAYLIST):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
        updating: true
      };
    case FAILURE(ACTION_TYPES.FETCH_PLAYLIST_LIST):
    case FAILURE(ACTION_TYPES.FETCH_PLAYLIST):
    case FAILURE(ACTION_TYPES.CREATE_PLAYLIST):
    case FAILURE(ACTION_TYPES.UPDATE_PLAYLIST):
    case FAILURE(ACTION_TYPES.DELETE_PLAYLIST):
      return {
        ...state,
        loading: false,
        updating: false,
        updateSuccess: false,
        errorMessage: action.payload
      };
    case SUCCESS(ACTION_TYPES.FETCH_PLAYLIST_LIST):
      return {
        ...state,
        loading: false,
        entities: action.payload.data
      };
    case SUCCESS(ACTION_TYPES.FETCH_PLAYLIST):
      return {
        ...state,
        loading: false,
        entity: action.payload.data
      };
    case SUCCESS(ACTION_TYPES.CREATE_PLAYLIST):
    case SUCCESS(ACTION_TYPES.UPDATE_PLAYLIST):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
        entity: action.payload.data
      };
    case SUCCESS(ACTION_TYPES.DELETE_PLAYLIST):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
        entity: {}
      };
    case ACTION_TYPES.RESET:
      return {
        ...initialState
      };
    default:
      return state;
  }
};

const apiUrl = 'api/playlists';

// Actions

export const getEntities: ICrudGetAllAction<IPlaylist> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_PLAYLIST_LIST,
  payload: axios.get<IPlaylist>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});

export const getEntity: ICrudGetAction<IPlaylist> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_PLAYLIST,
    payload: axios.get<IPlaylist>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<IPlaylist> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.CREATE_PLAYLIST,
    payload: axios.post(apiUrl, cleanEntity(entity))
  });
  dispatch(getEntities());
  return result;
};

export const updateEntity: ICrudPutAction<IPlaylist> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.UPDATE_PLAYLIST,
    payload: axios.put(apiUrl, cleanEntity(entity))
  });
  dispatch(getEntities());
  return result;
};

export const deleteEntity: ICrudDeleteAction<IPlaylist> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
    type: ACTION_TYPES.DELETE_PLAYLIST,
    payload: axios.delete(requestUrl)
  });
  dispatch(getEntities());
  return result;
};

export const reset = () => ({
  type: ACTION_TYPES.RESET
});
