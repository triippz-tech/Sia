...
public class AntiinviteCmd extends Command
{
    ...
    protected void execute(CommandEvent event)
    {
        ...
        sia.getDatabaseManagers().getAutoModService().setInviteStrikes(event.getGuild(), numstrikes);
        boolean also = sia.getDatabaseManagers().getActionsService().useDefaultSettings(event.getGuild());
        event.replySuccess("Users will now receive `"+numstrikes+"` strikes for posting invite links."+(also ? Punishment.DEFAULT_SETUP_MESSAGE : ""));
    }
}
