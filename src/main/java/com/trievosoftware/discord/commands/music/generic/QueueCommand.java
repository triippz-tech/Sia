...
public class QueueCommand extends AbstractMusicCommand
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        if(list.isEmpty())
        {
            ...
            {
                if(nowp!=null)
                    sia.getNowplayingHandler().setLastNPMessage(m);
            });
            return;
        }
        ...
        long fintotal = total;
        builder.setText((i1,i2) -> getQueueTitle(ah, event.getClient().getSuccess(), songs.length, fintotal, settings.isRepeat()))
            .setItems(songs)
    ...
    private String getQueueTitle(AudioHandler ah, String success, int songslength, long total, boolean repeatmode)
    {
        ...
        {
            sb.append(ah.getPlayer().isPaused() ? Constants.Music.PAUSE_EMOJI : Constants.Music.PLAY_EMOJI).append(" **")
