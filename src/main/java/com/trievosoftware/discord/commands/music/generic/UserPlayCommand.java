...
public class UserPlayCommand extends AbstractMusicCommand
{
    ...
    public UserPlayCommand(Sia sia, String loadingEmoji) {
        super(sia);
        ...
        this.children = new Command[]{new PlaylistCmd(sia)};
    }
    ...
    public void doCommand(CommandEvent event)
    {
        if(event.getArgs().isEmpty() && event.getMessage().getAttachments().isEmpty())
        {
            ...
            if(handler.getPlayer().getPlayingTrack()!=null && handler.getPlayer().isPaused())
            {
                ...
                if(!isDJ)
                    isDJ = event.isOwner();
//                GuildMusicSettings settings = event.getClient().getSettingsFor(event.getGuild());
    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        public void noMatches()
        {
            if(ytsearch)
                ...
                sia.getPlayerManager().loadItemOrdered(event.getGuild(), "ytsearch:"+event.getArgs(), new ResultHandler(m,event,true));
        }
    ...
    public class PlaylistCmd extends AbstractMusicCommand
    {
        ...
        public void doCommand(CommandEvent event) throws NoPlaylistFoundException {
            ...
            String playlistName = event.getArgs().trim().replaceAll(" ", "_").toUpperCase();
            PlaylistLoader.DiscordPlaylist playlist = sia.getPlaylistLoader().getUserPlaylist(event.getGuild(), playlistName, event.getAuthor());
            if(playlist==null)
            {
                event.replyError("I could not find `"+event.getArgs()+ "'.");
                return;
            }
            event.getChannel().sendMessage(loadingEmoji+" Loading playlist **"+playlistName+"**... ("+playlist.getItems().size()+" items)").queue(m ->
            {
                AudioHandler handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
                playlist.loadTracks(event.getGuild(), sia.getPlayerManager(), (at)->handler.addTrack(new QueuedTrack( at, event.getAuthor())), () -> {
