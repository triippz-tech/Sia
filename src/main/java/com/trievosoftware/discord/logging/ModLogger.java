...
import com.trievosoftware.discord.database.managers.GuildSettingsDataManager.GuildSettings;
...
public class ModLogger
{
    ...
    {
        if(sia.getDatabase().settings.getSettings(guild).getModLogChannel(guild)==null)
            return;
    ...
    {
        TextChannel modlog = sia.getDatabase().settings.getSettings(guild).getModLogChannel(guild);
        if(modlog==null)
            return -1;
    ...
    {
        TextChannel modlog = sia.getDatabase().settings.getSettings(moderator.getGuild()).getModLogChannel(moderator.getGuild());
            ...
            try
            {
                modlog.sendMessage(new MessageBuilder()
                        ...
                        .append(FormatUtil.filterEveryone(LogUtil.modlogCleanFormat(now, 
                                sia.getDatabase().settings.getSettings(moderator.getGuild()).getTimezone(),
    ...
    public void postStrikeCase(Member moderator, OffsetDateTime now, int givenStrikes, int oldStrikes, int newStrikes, User target, String reason)
    {
        ...
        getCaseNumberAsync(modlog, i -> 
        {
            modlog.sendMessage(FormatUtil.filterEveryone(LogUtil.modlogStrikeFormat(now, 
                    sia.getDatabase().settings.getSettings(moderator.getGuild()).getTimezone(), i,
    ...
    public void postPseudoCase(Member moderator, OffsetDateTime now, Action act, User target, int minutes, String reason)
    {
        ...
        if(modlog==null || !modlog.canTalk())
            return;
        ZoneId timezone = sia.getDatabase().settings.getSettings(moderator.getGuild()).getTimezone();
        getCaseNumberAsync(modlog, i -> 
        {
    ...
    private void update(Guild guild, int limit) // not async
    {
        if(guild==null)
            return;
        GuildSettings gs = sia.getDatabase().settings.getSettings(guild);
        ...
        try
        {
            ...
            for(AuditLogEntry ale: sia.getDatabase().auditcache.filterUncheckedEntries(list))
            {
                Action act = null;
                switch(ale.getType())
                {
                ...
                if(act!=null)
                {
                    ...
                    ZoneId timezone = sia.getDatabase().settings.getSettings(guild).getTimezone();
                    if(mod.isBot())
                    {
                    ...
                    if(act==Action.UNBAN) // check for softban
                    {
                        ...
                        sia.getDatabase().tempbans.clearBan(guild, ale.getTargetIdLong());
                    }
                    if(act==Action.UNMUTE)
                    {
                        sia.getDatabase().tempmutes.removeMute(guild, ale.getTargetIdLong());
                    }
